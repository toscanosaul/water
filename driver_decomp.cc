...
int main(int argc, char** argv)
{
    ...
    // Setup: num processors and points for each subdomain
    int NT = omp_get_num_threads();
    int npx = int(floor(sqrt(real(NT))));
    int npy = int(floor(sqrt(real(NT))));
    int nx_subd = nx/npx;
    int nx_subdg = nx_subd+2*nghost;
    int ny_subdg = ny_subd+2*nghost;
    int ny_subd = nx/npy;
